...
async def create_user(user: UserDB):
    try:
        ...
        return Response(
            status_code=status.HTTP_400_BAD_REQUEST, content="Something wrong!"
        )
    # the Id of the user
...
async def send_verification_to_mail(email_receiver: str):
    ...
    authentication_link = auth_admin.generate_email_verification_link(
...
async def sign_up(account: Account):
    ...
    user_id = auth.sign_in_with_email_and_password(email=email, password=password)[
        "localId"
    ]
